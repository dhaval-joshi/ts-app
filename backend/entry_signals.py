...
def _true_range(h: float, l: float, c_prev: float) -> float:
    return max(h - l, abs(h - c_prev), abs(l - c_prev))


def _ttm_squeeze_active(history: list[dict], period: int, num_std: float, kc_mult: float) -> bool | None:
    if len(history) < period + 1:
        return None
    
    # We need the last `period` bars for the moving average and std dev, plus one prior bar for True Range
    ...
    std = variance ** 0.5
    bb_half_width = num_std * std
    
    # Calculate True Range for the window
    true_ranges = []
    # window is exactly `period` long, we need the `c_prev` for the first element in window
    # which is history[-(period + 1)]["close"]
    for i in range(period):
        bar = window[i]
        c_prev = history[-(period + 1) + i]["close"]
        true_ranges.append(_true_range(bar["high"], bar["low"], c_prev))
        
    atr = sum(true_ranges) / period
    kc_half_width = kc_mult * atr
    
    # Squeeze is active if Bollinger Bands are strictly inside Keltner Channels
    return bb_half_width < kc_half_width


def squeeze_gate(history: list[dict], period: int, num_std: float, min_days: int,
                  kc_mult: float, require_ttm_squeeze: bool) -> tuple[bool, str | None]:
    """Skip unless price has been genuinely compressed over the last
    `period` days, defined by the John Carter TTM Squeeze (Bollinger Bands
    falling inside Keltner Channels). `history` is this Program's underlying's
    daily OHLCV price history (chronological, oldest first)."""
    if not require_ttm_squeeze:
        return True, None
        
    # Check if we have enough days built up to evaluate
    if len(history) < period + min_days:
        return True, None
        
    is_squeezed = _ttm_squeeze_active(history, period, num_std, kc_mult)
    if is_squeezed is None:
        return True, None  # not enough data or zero mean
        
    if not is_squeezed:
        return False, (f"Price is not compressed (TTM Squeeze inactive) -- Bollinger Bands "
                       f"({num_std} std) are not inside Keltner Channels ({kc_mult} ATR) over "
...
def evaluate_entry(cfg: dict, *, index_snapshot: dict | None, ce_snapshot: dict | None, pe_snapshot: dict | None,
    ...
    checks = [
        ...
        squeeze_gate(
            index_history,
            cfg.get("squeeze_bollinger_period", 20),
            cfg.get("squeeze_bollinger_std", 2.0),
            cfg.get("squeeze_min_days", 10),
            cfg.get("squeeze_keltner_mult", 1.5),
            cfg.get("require_ttm_squeeze", False)
        ),
    ]
