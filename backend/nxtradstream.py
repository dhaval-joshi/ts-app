...
def commafmt(value, precision=2):
    ...
    parts = v.split(".")
    # NOTE: this one-line fix (r"..." instead of "...") is a deliberate,
    # documented exception to "vendored, unmodified" above -- \B and \d
    # aren't real Python string escapes, so Python currently just warns
    # ("SyntaxWarning: invalid escape sequence") but a future Python
    # version will make this a hard error. A raw string changes nothing
    # about behavior, only how the literal is parsed, so it's safe to
    # apply even to otherwise-untouched vendored code.
